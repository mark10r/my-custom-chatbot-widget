...
const sessionStorageKey = (chatbotId: string) => `optinbot_session_${chatbotId}`;

type PersistedMessage = {
    type: 'user' | 'bot' | 'rating';
    text: string;
    timestamp: string;
    rating?: 'up' | 'down';
    feedback?: string;
...
type PersistedSession = {
    ...
    lastSentUserMessageCount: number;
};

const loadPersistedSession = (chatbotId: string): PersistedSession | null => {
    if (!chatbotId) return null;
    try {
        const raw = localStorage.getItem(sessionStorageKey(chatbotId));
        if (!raw) return null;
        ...
        if (Date.now() - parsed.lastActivityAt > SESSION_TTL_MS) {
            localStorage.removeItem(sessionStorageKey(chatbotId));
            return null;
        }
        return parsed;
    } catch {
...
const savePersistedSession = (chatbotId: string, data: Omit<PersistedSession, 'lastActivityAt'> & { lastActivityAt?: number }) => {
    if (!chatbotId) return;
    try {
        const payload: PersistedSession = { ...data, lastActivityAt: Date.now() };
        localStorage.setItem(sessionStorageKey(chatbotId), JSON.stringify(payload));
    } catch { /* localStorage unavailable */ }
};

// Build the running transcript in the same format used by the pagehide beacon
// so the server sees a consistent shape whether the message arrived mid-chat
...
const buildTranscript = (chatbotId: string, sessionId: string, msgs: TranscriptMessage[]): string => {
    const lines = msgs.map(msg => {
        ...
        return `**${msg.type === 'user' ? 'User' : 'Agent'}:** ${msg.type === 'bot' ? stripTranscriptHtml(msg.text) : msg.text}`;
    });
    return `Chatbot ID: ${chatbotId}\nSession ID: ${sessionId}\n\n${lines.join('\n')}`;
};
...
const ChatWidget: React.FC<ChatWidgetProps> = ({
    ...
    const [messages, setMessages] = useState<{ type: 'user' | 'bot' | 'rating'; text: string; timestamp: Date; rating?: 'up' | 'down'; feedback?: string; showBookingCard?: boolean }[]>(
        ...
        // stale picker.
        // Restored session wins. Otherwise ALWAYS seed the welcome message on
        // initial state — bubble mode's chat window is hidden until opened so
        // visitors don't see it early, inline mode shows it immediately. Prior
    ...
    const [hasRated, setHasRated] = useState(restored?.hasRated ?? false);
    const lastSentUserMessageCountRef = useRef<number>(restored?.lastSentUserMessageCount ?? 0);
    ...
    // Skip the API call if we already have a sessionId (either from restore or a prior call).
    useEffect(() => {
        if (!sessionId) {
    ...
    // --- EFFECT: PERSIST SESSION TO LOCALSTORAGE (24h TTL) ---
    // Runs on every message / hasRated / sessionId change. Skipped in preview mode.
    useEffect(() => {
    ...
    useEffect(() => {
        const handlePageLeave = () => {
            ...
            if (userMessageCount <= lastSentUserMessageCountRef.current) return;
            if (!isPreview && userMessageCount > 0 && sessionId) {
                const transcript = buildTranscript(chatbotId, sessionId, messages);
                ...
                try {
                    ...
                    if (chatbotId) {
                        savePersistedSession(chatbotId, {
                            ...
                            hasRated,
                            lastSentUserMessageCount: userMessageCount,
                        });
                    }
                } catch (e) {
    ...
    const handleSendMessage = async (messageText?: string) => {
        ...
        // even if the closing sendBeacon never makes it.
        const transcriptSoFar = buildTranscript(chatbotId, sessionId, [...messages, userMessage]);
